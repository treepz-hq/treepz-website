...
export const submitReport = async (
  ...
  if (!baseUrl) {
    ...
    return {
      success: false,
      message: 'API endpoint is not available. Please try again later.',
    };
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // const errorMessage = errorData?.message || "Submission failed. Try again.";
      // return { success: false, message: errorMessage };
      return {
        success: false,
        message: errorData?.message || 'Submission failed. Try again.',
      };
    }
